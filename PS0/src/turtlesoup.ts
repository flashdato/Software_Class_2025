...
export function chordLength(radius: number, angleInDegrees: number): number {
  ...
  return parseFloat((2 * radius * Math.sin(angleInRadians / 2)).toFixed(3));
...
export function findPath(turtle: Turtle, points: Point[]): string[] {
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    instructions.push(`forward ${distance}`);
...
export function main(): void {
  ...
  //  const pointsToVisit: Point[] = [{x: 0, y: 50 }, {x: 50, y: 50 }, {x: 50, y: 0 }];
  //  const pointsToVisit: Point[] = [{x: -10, y: -10 }, {x: 10, y: -10 }, {x: 10, y: 10 }];
  // const pointsToVisit: Point[] = [{x: 100, y: 100 }, {x: 200, y: 200 }, {x: 100, y: 300 }];
